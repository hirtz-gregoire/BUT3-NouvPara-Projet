...
public class MovieByGenre extends Page {

    public MovieByGenre(App app, Optional<DB> db){
        super(app, db);
    }

    protected void show(){
        String value = """
                Choisir les genres du filtre
    ...
    protected void exec(String entry){
        List<String> genres = Arrays.stream(entry.split(" ")).toList();
        List<Document> res = db.get().getFilmsByGenre(genres);
